...
export default function Home() {
  ...
  const filtered = useMemo(() => sections.filter((section) => section.label.toLowerCase().includes(query.toLowerCase())), [query]);
  ...
  return <div className="app-shell">
    <header className="mobile-header">
      <a className="brand" href="#inicio" onClick={() => goTo("inicio")}><img src="/manus-storage/mpt-mark_185d043a.png" alt="" /><span>MoneyPrinter<span>Turbo</span></span></a>
      <button className="mobile-menu" onClick={() => setMobileNav(!mobileNav)} aria-label="Abrir navegación">{mobileNav ? <X /> : <Menu />}</button>
    </header>
    <aside className={mobileNav ? "sidebar open" : "sidebar"}>
      <div className="sidebar-top"><a className="brand" href="#inicio" onClick={() => goTo("inicio")}><img src="/manus-storage/mpt-mark_185d043a.png" alt="" /><span>MoneyPrinter<span>Turbo</span></span></a><span className="version">MANUAL · ES</span></div>
      <div className="search-box"><Search size={16} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar sección" aria-label="Buscar sección" /></div>
      <nav className="side-nav" aria-label="Navegación del manual">{filtered.map(({ id, label, icon: Icon }) => <button key={id} className={active === id ? "nav-item active" : "nav-item"} onClick={() => goTo(id)}><Icon size={16} /><span>{label}</span>{active === id && <ChevronRight size={15} />}</button>)}</nav>
    ...
    <main className="main-content">
      <section className="hero-section" id="inicio">
        ...
        <div className="hero-copy"><div className="hero-brand-lockup"><img src="/manus-storage/mpt-mark_185d043a.png" alt="" /><span><strong>MoneyPrinter<span>Turbo</span></strong><small>MANUAL DE VIDEO AUTOMATIZADO</small></span></div><div className="eyebrow"><span className="eyebrow-line" /> DOCUMENTACIÓN ESENCIAL</div><h1>De una idea<br /><em>a un video.</em></h1><p>La guía práctica para instalar, configurar y automatizar <strong>MoneyPrinterTurbo</strong>, el generador de videos cortos con IA.</p><div className="hero-actions"><button className="primary-btn" onClick={() => goTo("instalacion")}>Comenzar guía <ArrowUpRight size={17} /></button><a className="text-link" href="https://github.com/harry0703/MoneyPrinterTurbo/blob/main/README-en.md" target="_blank" rel="noreferrer">README original <ExternalLink size={14} /></a></div></div>
