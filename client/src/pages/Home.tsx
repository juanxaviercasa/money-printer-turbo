...
import {
  ...
  Code2, Cpu, ExternalLink, FileCode2, Github, History, Menu, Monitor,
  Package, Play, Plus, Search, Server, Sparkles, Terminal, Trash2, X, Zap,
} from "lucide-react";
...
const sections = [
  ...
  { id: "historial", label: "Historial local", icon: History },
  { id: "funciones", label: "Funciones", icon: Zap },
  { id: "problemas", label: "Solución de problemas", icon: CircleHelp },
];

const searchKeywords: Record<string, string> = { inicio: "visión general tema video guía", requisitos: "cpu ram gpu python sistema", instalacion: "windows macos linux docker uv pip", nube: "cloud vps railway colab servidor", configuracion: "voz tts whisper subtítulos formato", proveedores: "llm api claves openai gemini deepseek kimi azure", automatizacion: "cron webhook schedule programada upload tiktok instagram youtube", historial: "historial videos generados estados ready failed rendering local", funciones: "flujo cli guion montaje publicación", problemas: "ffmpeg whisper archivos errores soporte" };

const installationSteps = ["Elegir ruta: paquete de Windows, manual con uv/pip o Docker", "Instalar dependencias y completar el primer arranque", "Configurar proveedor LLM/TTS y guardar las claves en el servidor", "Generar un video de prueba y revisar salida, voz y subtítulos", "Preparar cron, webhook o publicación programada si automatizas"];
...
export default function Home() {
  ...
  const [platform] = useState(() => { const ua = navigator.userAgent.toLowerCase(); return ua.includes("win") ? "Windows" : ua.includes("mac") ? "macOS" : ua.includes("linux") ? "Linux" : "tu sistema"; });
  const [videoHistory, setVideoHistory] = useState<{ topic: string; status: string; date: string }[]>(() => { try { const saved = localStorage.getItem("videoforge-video-history"); return saved ? JSON.parse(saved) : []; } catch { return []; } });
  const [newTopic, setNewTopic] = useState("");
  const addHistoryItem = () => { const topic = newTopic.trim(); if (!topic) return; const next = [{ topic, status: "ready", date: new Date().toLocaleDateString("es-ES") }, ...videoHistory].slice(0, 8); setVideoHistory(next); localStorage.setItem("videoforge-video-history", JSON.stringify(next)); setNewTopic(""); toast.success("Video añadido al historial local"); };
  const clearHistory = () => { setVideoHistory([]); localStorage.removeItem("videoforge-video-history"); toast.success("Historial local limpiado"); };
  const [checkedSteps, setCheckedSteps] = useState<boolean[]>(() => { try { const saved = localStorage.getItem("videoforge-install-checklist"); return saved ? JSON.parse(saved) : installationSteps.map(() => false); } catch { return installationSteps.map(() => false); } });
  const completedSteps = checkedSteps.filter(Boolean).length;
  const toggleStep = (index: number) => setCheckedSteps((current) => { const next = current.map((checked, step) => step === index ? !checked : checked); localStorage.setItem("videoforge-install-checklist", JSON.stringify(next)); return next; });
  const resetChecklist = () => { const empty = installationSteps.map(() => false); localStorage.setItem("videoforge-install-checklist", JSON.stringify(empty)); setCheckedSteps(empty); };
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="content-frame">
        ...
        <section className="doc-section" id="instalacion"><div className="section-kicker"><span className="section-signal" />03 / PUESTA EN MARCHA</div><div className="section-heading"><h2>Elige tu ruta de instalación</h2><p>Empieza por la opción que mejor encaje con tu experiencia y objetivo.</p></div><div className="os-recommendation"><Monitor size={19} /><span><strong>Detectamos: {platform}</strong><small>Esta guía destaca la ruta más cercana a tu equipo; puedes elegir otra cuando quieras.</small></span><a href="#instalacion">Ver ruta recomendada <ArrowUpRight size={14} /></a></div><div className="route-grid"><article className="route-card featured"><div className="route-top"><span className="route-num">01</span><span className="tag">MÁS SIMPLE</span></div><h3>Windows · Un clic</h3><p>La forma más rápida de hacer una primera prueba local sin configurar Python manualmente.</p><ol><li>Descarga el paquete desde <a href="https://github.com/harry0703/MoneyPrinterTurbo/releases/latest" target="_blank" rel="noreferrer">GitHub Releases <ExternalLink size={12} /></a>.</li><li>Extrae el archivo y ejecuta <code>update.bat</code>.</li><li>Ejecuta <code>start.bat</code>. Chrome o Edge abrirán la interfaz.</li></ol><a className="route-link" href="https://github.com/harry0703/MoneyPrinterTurbo/releases/latest" target="_blank" rel="noreferrer">Descargar paquete <ArrowUpRight size={15} /></a></article><article className="route-card"><div className="route-top"><span className="route-num">02</span><span className="tag muted-tag">CONTROL TOTAL</span></div><h3>macOS / Linux · Manual</h3><p>La ruta recomendada si quieres entender el entorno, automatizar por CLI o mantenerlo actualizado.</p><CodeBlock label="1. Clonar y preparar entorno" value={commands.clone + "\n\n" + commands.uv} /><CodeBlock label="2. Lanzar la WebUI" value={`sh webui.sh\n\n# Acceso LAN (opcional)\nMPT_WEBUI_HOST=0.0.0.0 sh webui.sh`} /></article></div><div className="subroute"><div><strong>¿Prefieres pip?</strong><span>Usa venv como alternativa a uv.</span></div><CodeBlock label="Entorno clásico" value={commands.pip} /></div><div className="install-checklist"><div className="checklist-head"><div><span className="card-index">CHECKLIST · INSTALACIÓN</span><h3>Marca cada hito</h3></div><div className="checklist-progress"><strong>{completedSteps}/{installationSteps.length}</strong><span>completados</span></div></div><div className="progress-track"><i style={{ width: `${(completedSteps / installationSteps.length) * 100}%` }} /></div><div className="checklist-items">{installationSteps.map((step, index) => <label key={step} className={checkedSteps[index] ? "check-item done" : "check-item"}><input type="checkbox" checked={checkedSteps[index]} onChange={() => toggleStep(index)} /><span className="custom-check">{checkedSteps[index] && <Check size={13} />}</span><span>{step}</span></label>)}</div><button className="reset-checklist" onClick={resetChecklist}>Reiniciar checklist</button></div></section>
        ...
        <section className="doc-section" id="historial"><div className="section-kicker"><span className="section-signal" />08 / CONTROL LOCAL</div><div className="section-heading"><h2>Historial de videos</h2><p>Un registro local y privado para recordar qué temas generaste y en qué estado quedaron.</p></div><div className="history-panel"><div className="history-form"><div><span className="card-index">AÑADIR REGISTRO</span><h3>Guarda una referencia rápida</h3></div><div className="history-input"><input value={newTopic} onChange={(event) => setNewTopic(event.target.value)} onKeyDown={(event) => event.key === "Enter" && addHistoryItem()} placeholder="Ej. 5 hábitos para dormir mejor" aria-label="Tema del video" /><button onClick={addHistoryItem} aria-label="Añadir video"><Plus size={16} /></button></div><small>Este historial solo se guarda en el navegador. No genera videos ni sube datos a un servidor.</small></div><div className="history-list">{videoHistory.length ? videoHistory.map((item, index) => <div className="history-item" key={`${item.topic}-${index}`}><span className="history-status">{item.status}</span><strong>{item.topic}</strong><small>{item.date}</small></div>) : <div className="history-empty"><History size={18} /><span>Aún no hay videos registrados. Añade el primero arriba.</span></div>}<button className="clear-history" onClick={clearHistory} disabled={!videoHistory.length}><Trash2 size={14} /> Limpiar historial</button></div></div></section>

        <section className="doc-section" id="funciones"><div className="section-kicker"><span className="section-signal" />09 / FLUJO DE TRABAJO</div><div className="section-heading"><h2>De tema a video publicado</h2><p>Cuatro pasos. Una misma interfaz.</p></div><div className="flow"><div className="flow-line" />{[["01", "Idea", "Escribe un tema o palabra clave."], ["02", "Guion", "La IA redacta el texto y extrae palabras clave."], ["03", "Montaje", "Se sincronizan clips, voz, subtítulos y música."], ["04", "Publicación", "Exporta o publica en TikTok, Instagram y YouTube Shorts."]].map(([num, title, desc]) => <div className="flow-step" key={num}><span>{num}</span><div><h3>{title}</h3><p>{desc}</p></div></div>)}</div><div className="cli-callout"><div><div className="section-kicker"><span className="section-signal" />ATAJO · CLI</div><h3>Genera sin abrir el navegador</h3><p>Ideal para scripts, cron jobs y pipelines de automatización.</p></div><CodeBlock label="Terminal" value={commands.cli} /></div></section>

        <section className="doc-section" id="problemas"><div className="section-kicker"><span className="section-signal" />10 / SOPORTE</div><div className="section-heading"><h2>Solución de problemas</h2><p>Los tres errores más frecuentes durante la puesta en marcha.</p></div><div className="trouble-list"><details open><summary><span><span className="error-dot" />No se encuentra FFmpeg</span><ChevronRight size={17} /></summary><div><p>Normalmente se descarga automáticamente. Si falla, instala FFmpeg y define su ruta en <code>config.toml</code>:</p><CodeBlock label="config.toml · Windows" value={`[app]\nffmpeg_path = "C:\\\\Users\\\\tu_usuario\\\\Downloads\\\\ffmpeg.exe"`} /></div></details><details><summary><span><span className="error-dot" />Too many open files</span></summary><div><p>Comprueba el límite actual y elévalo temporalmente:</p><CodeBlock label="Terminal" value={`ulimit -n\nulimit -n 10240`} /></div></details><details><summary><span><span className="error-dot" />No descarga el modelo Whisper</span></summary><div><p>Descarga el modelo desde Hugging Face y coloca la carpeta completa dentro de <code>MoneyPrinterTurbo/models</code>.</p><a href="https://huggingface.co/Systran/faster-whisper-large-v3" target="_blank" rel="noreferrer">Abrir modelo Whisper <ExternalLink size={13} /></a></div></details></div></section>
